  ...
  describe("#cardValidator", function() {
    ...
    describe("When haven't parameter", function() {
      it("should throw an error", () => {
        let badFn = () => { index.cardValidator() };
        expect(badFn).to.throw("You must enter a parameter!");
      });
    })
    ...
    describe("When number is a string", function() {
      it("should throw an error", () => {
        let badFn = () => { index.cardValidator("hello world") };
        expect(badFn).to.throw("Please insert only numbers!");
      });
    })

    describe("When number is a integer but only has one digit", function() {
      it("should throw an error", () => {
        let badFn = () => { index.cardValidator(7) };
        expect(badFn).to.throw("The value insert has only one digit, please insert your complete card number!");
      });
    })
